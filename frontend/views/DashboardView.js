...
export const DashboardView = {
  setup() {
    ...
    onMounted(async () => {
      try {
        if (isPatient) {
          // Fetch profile (has totalRecords, totalVersions, activeCollaborators)
          ...
          recentActivity.value = (commitRes.commits || []).slice(0, 5).map(c => ({
            ...
            resource: c.commitMessage || c.title,
          ...
          // Use recent notifications as activity feed
    ...
    const activityColor = (type) => ({
      doctor:  'text-sage-400',
      self:    'text-ink-400',
      request: 'text-rust-400',
      approve: 'text-sage-500',
      endorse: 'text-amber-400',
    }[type] || 'text-ink-500');
    ...
    <div class="p-6 max-w-5xl mx-auto animate-fade-in">
      <div class="mb-8">
        ...
        <h1 class="serif text-3xl text-ink-100">
          Good day, {{ (appState.user?.displayName || '').split(' ')[0] }}
        </h1>
      ...
      <div v-else-if="stats && !isPatient" class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
        <div class="stat-card">
          <div class="mono text-2xl text-ink-100 font-medium">{{ stats.totalCasesHandled }}</div>
          <div class="mono text-[10px] text-ink-600 mt-1">Total Cases</div>
        </div>
        <div class="stat-card">
          <div class="mono text-2xl text-sage-400 font-medium">{{ stats.activeCases }}</div>
          <div class="mono text-[10px] text-ink-600 mt-1">Active</div>
        </div>
        <div class="stat-card">
          <div class="mono text-2xl text-ink-100 font-medium">{{ stats.averageResponseTimeHours }}h</div>
          <div class="mono text-[10px] text-ink-600 mt-1">Avg Response</div>
        </div>
        <div class="stat-card">
          <div class="mono text-2xl text-sage-400 font-medium">{{ stats.recordAccuracyScore }}</div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mt-4">
        <button v-if="isPatient"  @click="router.push('/records')"          class="card p-4 text-left hover:border-sage-800/40 transition-all">
        ...
        <button v-if="!isPatient" @click="router.push('/access-requests')"  class="card p-4 text-left hover:border-sage-800/40 transition-all">
        ...
        <button @click="router.push('/doctors')" class="card p-4 text-left hover:border-sage-800/40 transition-all">
        ...
        <button @click="router.push('/notifications')" class="card p-4 text-left hover:border-sage-800/40 transition-all">
          <div class="text-ink-400 text-xl mb-1">◉</div>
